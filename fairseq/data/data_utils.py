...
def load_img_features(imag_npy_path):
    grid_img_npy = np.load(imag_npy_path + '.npy', allow_pickle=True)
    ...
    region_img_features = open(imag_npy_path + '.pkl', 'rb')
    region_img_features = pickle.load(region_img_features)
    # region_img_features = torch.tensor(region_img_features)
    # list_len = [len(i) for i in region_img_features]
    # max_len = max(list_len)
    # # padding_zeros = np.zeros((5,2048), dtype=np.float32)
    # region_img_features_tmp = []
    # for img in region_img_features:
    #     padding_num = (max_len - len(img))
    #     padding_zeros = torch.tensor(np.zeros((padding_num, 2048), dtype=np.float32))
    #     img_padded_tmp = torch.cat([torch.tensor(img), padding_zeros])
