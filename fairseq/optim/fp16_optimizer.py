...
class DynamicLossScaler(object):
    ...
    def update_scale(self, overflow):
        iter_since_rescale = self._iter - self._last_rescale_iter   #1
        if overflow:
            self._last_overflow_iter = self._iter
            self._overflows_since_rescale += 1                      #0+1
            pct_overflow = self._overflows_since_rescale / float(iter_since_rescale) #1
